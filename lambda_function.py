...
def lambda_handler(event, context):
    ...
    def upload_via_public_url(page_id, page_token, file_id, title, description):
        """Facebook Reels API ke zariye Google Drive ka public URL pass kar ke Reels post karo."""
        # 1. Initialize Reels upload session
        ...
        try:
            ...
            print(f"Error during Reels initialization: {e}")
            return {"error": f"Initialization failed: {str(e)}"}

        if "video_id" not in init_data or "upload_url" not in init_data:
            print(f"Reels initialization response error: {init_data}")
            return init_data

        video_id = init_data["video_id"]
        upload_url = init_data["upload_url"]

        # 2. Upload video file via URL using file_url header
        ...
        try:
            upload_res = requests.post(upload_url, headers=headers, timeout=120)
            upload_data = upload_res.json()
        except Exception as e:
            print(f"Error during Reels video upload: {e}")
            return {"error": f"Upload failed: {str(e)}"}

        if not upload_data.get("success"):
            print(f"Reels upload response error: {upload_data}")
            return upload_data

        # 3. Finalize and publish Reel
        publish_payload = {
            'upload_phase': 'finish',
            'video_id': video_id,
        ...
        try:
            publish_res = requests.post(init_url, data=publish_payload, timeout=30)
            publish_data = publish_res.json()
            if publish_data.get("success") or "id" in publish_data:
                # Return {"id": video_id} to keep compatibility with the rest of the script
                return {"id": video_id, "success": True}
            else:
                print(f"Reels publish response error: {publish_data}")
                return publish_data
        except Exception as e:
            print(f"Error during Reels finalize: {e}")
